...
def parse_file(file_path, max_round):
    ...
    The function now ignores any new sections such as the factor threshold (fit) output.
    """
    loss_data = {}
    acc_data = {}
    ...
    for line in lines:
        if "History (loss, distributed):" in line:
            ...
            continue
        if in_loss_section:
            # Stop processing loss section if we hit a new header (that isn't a round value)
            if "History (" in line and "round" not in line:
                break
    ...
    # Parse accuracy values from the evaluate section only.
    ...
    for line in lines:
        if "History (metrics, distributed, evaluate):" in line:
            ...
            continue
        if in_eval_section:
            # If we see a new header that isn't part of the dictionary, stop accumulating.
            if "History (" in line and "np.float64(" not in line:
                break
            eval_lines.append(line.strip())
    eval_text = " ".join(eval_lines)
    # Use regex to extract tuples of the form: (round, np.float64(value))
    acc_matches = re.findall(r'\((\d+),\s*np\.float64\(([\d\.eE+-]+)\)\)', eval_text)
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert multiple txt files into loss.csv and accuracy.csv (ignoring new factor threshold outputs)."
    )
